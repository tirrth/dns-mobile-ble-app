...
const Slider = ({
  ...
  const {inPropsRef, setLow, setHigh} = useLowHigh(
    lowProp,
  ...
  // To change the position of selected rail when changing the value of lowProp/highProp
  useEffect(() => {
    updateSelectedRail();
    updateThumbs();
  }, [lowProp, highProp]);
  ...
  useEffect(() => {
    const {low, high} = inPropsRef.current;
    if (
      (lowProp !== undefined && lowProp !== low) ||
      (highProp !== undefined && highProp !== high)
    ) {
      updateThumbs();
    }
